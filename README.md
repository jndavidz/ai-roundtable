...
- **回复聚合** - 不经 AI 加工，直接把各模型回复原文收集到侧栏，可一键复制
...
只是把各模型的回复原文收集到一起，直接在侧栏查看、复制：
1. 先让多个 AI 各自回答
2. 点击工具栏「**聚合**」按钮
3. 侧栏下方展示每位参与者的回复原文（按勾选顺序），点「复制全文」可复制纯文本

> 区别：`/summary` 是让一个模型**综合分析**（提炼共识与分歧）；「聚合」是**原文汇集**，零加工，适合自己通读或粘贴到笔记。
